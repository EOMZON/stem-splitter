...
在支持 Python 3.10 和 ffmpeg 的 macOS / Linux 上，只要：
...
上传一首歌即可看到分轨进度和结果页面。  
（首次运行会自动创建虚拟环境并安装 Demucs，时间会稍长一些。）
...
- `scripts/run_local.sh`  
  本地一键启动脚本：创建 `venv/`、安装 Flask 依赖，并启动 Flask 应用。
...
如果不使用 `scripts/run_local.sh`，也可以手动按以下步骤运行：
